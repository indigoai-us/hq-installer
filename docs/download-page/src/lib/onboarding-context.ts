 ...
 *
 * NOTE: this mirrors `src/lib/onboarding-context.ts` in the main repo. The
 * duplication is deliberate — the download page is a SEPARATE Astro/Vercel
 * build unit that cannot import across the repo boundary, and the main-repo
 * copy is the canonical, unit-tested source (the future desktop-app integration
 * imports that one). Keep the two in sync; both are tiny pure parsers.
 *
